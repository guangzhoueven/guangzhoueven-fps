...
function drawMinimap(){
  if(!mmCtx) return;
  const W=150, H=150, sc=2.2;
  const cx=camera.position.x, cz=camera.position.z;
  const ox=W/2 - cx*sc, oz=H/2 - cz*sc;
  mmCtx.clearRect(0,0,W,H);
  mmCtx.fillStyle='rgba(20,16,10,.5)'; mmCtx.fillRect(0,0,W,H);
  // Grid
  mmCtx.strokeStyle='rgba(249,115,22,.06)'; mmCtx.lineWidth=1;
  for(let i=0;i<=W;i+=20){mmCtx.beginPath();mmCtx.moveTo(i,0);mmCtx.lineTo(i,H);mmCtx.stroke();mmCtx.beginPath();mmCtx.moveTo(0,i);mmCtx.lineTo(W,i);mmCtx.stroke();}
  // Rooms
  mmCtx.strokeStyle='rgba(249,115,22,.3)'; mmCtx.lineWidth=1.5;
  for(const rm of ROOMS){
    const x=rm.x*sc+ox-(rm.w*sc)/2, z=rm.z*sc+oz-(rm.d*sc)/2;
    mmCtx.strokeRect(x,z,rm.w*sc,rm.d*sc);
  }
  // Walls
  ...
  for(const w of wallMeshes){
    ...
    const wx=(bb.min.x+bb.max.x)/2*sc+ox, wz=(bb.min.z+bb.max.z)/2*sc+oz;
    const ww=(bb.max.x-bb.min.x)*sc, wh=(bb.max.z-bb.min.z)*sc;
    if(wx<-5||wx>W+5||wz<-5||wz>H+5) continue;
    mmCtx.fillRect(wx-ww/2,wz-wh/2,Math.max(1,ww),Math.max(1,wh));
  }
  // Pickups
  ...
  for(const e of enemies){
    ...
    mmCtx.fillStyle = e.def.kind==='boss'?'#ff2222':e.def.kind==='brute'?'#aa44cc':e.def.kind==='shooter'?'#22aacc':e.def.kind==='runner'?'#ddaa22':'#cc3333';
    const r = e.def.kind==='boss'?5:e.def.kind==='brute'?3.5:2.5;
    mmCtx.beginPath(); mmCtx.arc(ex,ez,r,0,7); mmCtx.fill();
  }
  // Player triangle
  mmCtx.save(); mmCtx.translate(W/2,H/2); mmCtx.rotate(-yaw+Math.PI);
  mmCtx.fillStyle='#fbbf24'; mmCtx.shadowColor='#fbbf24'; mmCtx.shadowBlur=6;
  mmCtx.beginPath(); mmCtx.moveTo(0,-6); mmCtx.lineTo(-4,4); mmCtx.lineTo(0,2); mmCtx.lineTo(4,4); mmCtx.closePath(); mmCtx.fill();
  mmCtx.restore();
  // Border
  mmCtx.strokeStyle='rgba(249,115,22,.3)'; mmCtx.lineWidth=1; mmCtx.strokeRect(.5,.5,W-1,H-1);
}
